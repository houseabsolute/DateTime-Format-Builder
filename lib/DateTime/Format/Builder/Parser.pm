...
implementations and those used by C<Builder>. It is generally unlikely the user
will want to use any of them.
...
standard manner and due to C<Parser>'s implementation, these methods also allow
C<Parser> to determine which implementation to use.
...
Arguments are as per L<Params::Validate>'s C<validate> function. This method is
used to declare what your valid arguments are in a parser specification.
...
looked at and whichever module is the first to have a unique key in the spec is
the one to whom the spec is given.

B<Note>: please declare a C<valid_params> argument with an uppercase letter.
For example, if you're writing C<DateTime::Format::Builder::Parser::Fnord>,
declare a parameter called C<Fnord>. Similarly, C<DTFBP::Strptime> should have
C<Strptime> and C<DTFBP::Regex> should have C<Regex>. These latter two don't
...
parser specifications, this returns a coderef that returns either the resultant
C<DateTime> object or C<undef>.
...
Hash references B<with> I<length> keys are run through C<create_single_parser>,
but the resultant parser is used as the value in the length hashref with the
length being the key. If two or more parsers have the same I<length> specified
then an error is thrown.

=head2 create_parser

C<create_class> is mostly a wrapper around C<create_parser> that does loops and
stuff and calls C<create_parser> to create the actual parsers.

C<create_parser> takes the parser specifications (be they single specifications
or multiple specifications) and returns an anonymous coderef that is suitable
for use as a method. The coderef will call C<croak> in the event of being
unable to parse the single string it expects as input.

The simplest input is that of a single specification, presented just as a plain
hash, not a hashref. This is passed directly to C<create_single_parser> with
the return value from that being wrapped in a function that lets it C<croak> on
failure, with that wrapper being returned.

If the first argument to C<create_parser> is an arrayref, then that is taken to
be an options block (as per the multiple parser specification documented
earlier).
...
C<DateTime::Format::Builder::Parser::XXX> where I<XXX> is whatever you like, so
long as it doesn't start with a lower case letter.
...
the I<XXX> from earlier. This will be used to help identify which module a
...
C<preprocess>, C<on_match>, C<on_fail> and C<postprocess>. See the L<main
Builder|DateTime::Format::Builder> docs for the appropriate placing of calls to
the callbacks.
