...
This is provided to save time. It lets you do:
...
Thus it obviates part of the standard call style. However, anything it
uses in its parameters must be declared and defined at compile time.
...
C<length> is an optional parameter that can be used to specify that
this particular C<regex> is only applicable to strings of a certain
fixed length.  Our inefficient ical parser in the previous example
could be made more efficient by specifying the length for each
C<regex>.

For obvious reasons, you cannot specify the same C<length> twice for
one parser.
...
Any length calculations (for length-specific parsers) are done after
this preprocessing.
