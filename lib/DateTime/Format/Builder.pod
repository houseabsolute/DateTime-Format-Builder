...
After that, you call Builder with some options. Currently,
only I<parsers> is valid. 
...
a wrapper that will call C<die()> if the single parser
...
Similarly, any multiple parsers will only throw an error
...
In the tutorial I said there was only one parameter at
...
field is overridden so it is only of use when calling as
C<create_class()>.
